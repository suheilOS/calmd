...
Navigation uses an application-owned stack so asynchronous saves and conflicts can veto transitions. It preserves the composer thought, supports note-to-note traversal and back navigation, avoids duplicate self-navigation, and rewrites historical keys after an internal rename.
