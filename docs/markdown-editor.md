...
- The English application shell stays LTR. User-authored titles, composer input, search results, backlinks, mention excerpts, and preview blocks use independent `dir="auto"` boundaries so Arabic content does not inherit direction from nearby interface copy.
- CodeMirror assigns `dir="auto"` to every rendered Markdown line and enables per-line direction measurement. Arabic-first and Latin-first paragraphs can therefore coexist with correct leading-edge alignment, selection geometry, and visual cursor movement. Markdown markers remain part of the source but do not determine base direction because they are neutral characters.
- Inline code, URLs, standard links, and wiki links are Unicode bidi isolates. Code spans, URLs, list markers, task markers, and fenced code remain LTR. Editor prose and headings use Noto Naskh Arabic for Arabic glyphs while Latin stays in Rubik; Arabic inside code falls back from JetBrains Mono to Noto Naskh Arabic.
- Preview headings, paragraphs, list items, blockquotes, and table cells resolve direction separately. Callout titles and individual body lines also resolve independently, so mixed-direction callouts align each line to its own leading edge. Preview code stays LTR. Logical padding, margins, borders, and text alignment mirror from each resolved content boundary.
...
- Live Preview is the sole editing presentation. Inline syntax reveals only when a caret or completed selection touches its parsed span; structural prefixes reveal on their owning line. Carets activate inclusive boundaries, and a selection activates every syntax span it overlaps. GFM tables remain semantic tables while a focused cell edits the underlying Markdown. Tab and Shift-Tab move between cells, Enter moves down, and leaving the final cell adds a row. Restrained controls add rows or columns and provide a table-source escape hatch. Inactive Obsidian-compatible `> [!type]` callouts render as labeled callout blocks; clicking or moving the caret into a callout reveals its complete Markdown source. Opaque Live Preview surfaces render in a background layer below CodeMirror's drawn selection so selections remain visible across code, inline code, and highlighted text.
...
- Literal search folds Arabic harakat, Quranic annotation marks, and tatweel in the rebuildable FTS index. Queries with or without those marks match the same title or body, while note titles and result excerpts retain the original Unicode text.
...
- A floating toolbar appears after a non-empty body selection is completed. It exposes bold, italic, link, highlight, inline code, and strikethrough commands plus a compact block-style menu for paragraph, headings, quote, lists, and tasks. The toolbar uses the same Markdown-aware commands as the keyboard shortcuts and stays hidden during ordinary collapsed-caret editing and in the plain-text title. `Alt+F10` opens and focuses it for either a selection or a collapsed body caret.
- Typing `/` at line start or after whitespace opens a caret-anchored Insert menu for a callout, GFM table, fenced code block, or horizontal rule. A query filters the menu; typing whitespace or pressing Escape dismisses it without consuming the typed source. Block insertion replaces the slash query in one undoable edit. When text precedes the trigger, the inserted block moves below that content and receives the blank-line separation required by Markdown.
