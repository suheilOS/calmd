...
- Live Preview is the sole editing presentation. Inline syntax reveals only when a caret or completed selection touches its parsed span; structural prefixes reveal on their owning line. Carets activate inclusive boundaries, and a selection activates every syntax span it overlaps.
...
- Standard Markdown links show only their label while inactive. An ordinary click reveals editable source; primary Ctrl-click on Linux/Windows or Cmd-click on macOS opens absolute HTTP or HTTPS destinations.
