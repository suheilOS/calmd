...
export function BacklinksPopover({ noteKey, open, onOpenChange, onSelect }: BacklinksPopoverProps) {
  ...
  return (
    <Popover.Root open={open} onOpenChange={handleOpenChange}>
      <Popover.Trigger aria-label="Show backlinks" className="fixed bottom-5 right-5 z-10 inline-flex size-9 items-center justify-center rounded-full text-muted transition-[background-color,color,transform] duration-150 ease-out hover:bg-hover hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-faint active:scale-[0.97] sm:bottom-8 sm:right-8">
        <InfoIcon />
      </Popover.Trigger>
      <Popover.Portal>
        <Popover.Positioner align="end" positionMethod="fixed" side="top" sideOffset={8}>
          <Popover.Popup className="backlinks-popover w-64 max-w-[calc(100vw-2.5rem)] rounded-lg bg-surface p-2 text-ink shadow-[0_8px_24px_oklch(0_0_0/0.08)] outline-none">
            {error ? <p className="p-2 text-small text-secondary" role="alert">{error}</p> : backlinks === null ? (
              <p className="p-2 text-small text-faint">Loading…</p>
            ) : backlinks.length === 0 ? (
              <p className="p-2 text-small text-faint">No backlinks</p>
            ) : backlinks.map((link) => (
              <button className="block min-h-10 w-full rounded-md px-3 py-2 text-left text-small hover:bg-hover focus-visible:outline-2 focus-visible:outline-faint" key={link.key} onClick={() => onSelect(link.key)} type="button">
                {link.title}
              </button>
            ))}
          </Popover.Popup>
